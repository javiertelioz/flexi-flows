...
func (tn *TaskNode) Execute(ctx context.Context, wm *WorkflowManager, data interface{}) (interface{}, error) {
	...
	select {
	case <-ctx.Done():
		return nil, NewWorkflowError(tn.ID, tn.GetType(), "context cancelled before task execution", ctx.Err())
	default:
	}

	// Verificar que hay una tarea asignada
	if tn.Task == nil {
		return nil, NewWorkflowError(tn.ID, tn.GetType(), "no task function defined", fmt.Errorf("task is nil"))
	}
...
func (tn *TaskNode) executeTaskWithReflection(ctx context.Context, data interface{}) (interface{}, error) {
	taskValue := reflect.ValueOf(tn.Task)
	taskType := taskValue.Type()
	...
	if numOut != 2 {
		return nil, NewWorkflowError(tn.ID, tn.GetType(), "invalid task signature",
			fmt.Errorf("function must return (interface{}, error), got %d return values", numOut))
	}
